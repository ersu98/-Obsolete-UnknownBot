...
module.exports.run = async (bot, message, args) => {
  ...
  if (args[0]) {
      ...
      let tobe_translated = message.content.slice(prefix.length + args.length + 8) // Getting the text
      ...
      translate(tobe_translated, {
          ...
          message.channel.send(":x: Usage: `" + botconfig.prefix + "translate [from:iso] [to:iso] <some text>` \nThe from: and to: are optional, you can check out <http://bit.ly/ISO_codesWiki> for the iso codes\nExample: ```" + botconfig.prefix + "translate from:ro to:fr Salut, ce mai faci?```") // Yes, I used Romanian for my example. Do you have any problem?
      });
  } else {
      message.channel.send(":x: Usage: `" + botconfig.prefix + "translate [from:iso] [to:iso] <some text>` \nThe from: and to: are optional, you can check out <http://bit.ly/ISO_codesWiki> for the iso codes\nExample: ```" + botconfig.prefix + "translate from:ro to:fr Salut, ce mai faci?```")
  }
